...
totalPrice (MkShoppingEntry nimi hinta maara) = hinta * fromIntegral maara
...
buyOneMore (MkShoppingEntry nimi hinta maara) = MkShoppingEntry nimi hinta (maara + 1)
...
setName nimi (MkPerson name age) = MkPerson nimi age

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge ika (MkPerson name age) = MkPerson name ika
...
study (NthYear vuosi) = if vuosi < 7 then NthYear (vuosi +1) else Graduated
...
tick :: UpDown -> UpDown
tick (Up arvo) = Up (arvo +1)
tick (Down arvo) = Down (arvo -1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (Up arvo) = Down arvo
toggle (Down arvo) = Up arvo
...
oneExample :: OneOrTwo Bool
oneExample = One True

twoExample :: OneOrTwo String 
twoExample = Two "cat" "dog"
------------------------------------------------------------------------------
...
pairExample :: KeyVals String Bool
pairExample = Pair "cat" True (Pair "dog" False Empty)
