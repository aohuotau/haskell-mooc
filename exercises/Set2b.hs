...
oddFactorial 1 = 1
oddFactorial x = if x == 0 then 0 else if x `mod` 2 == 0 then oddFactorial (x-1) else x * oddFactorial (x-2)
...
myGcd 0 y = y
myGcd x 0 = x  
myGcd x y = if (x-y) > (y-x) then myGcd(x-y) y else myGcd x (y-x)
...
leftpad s x = if length s >= x then s else leftpad (" " ++ s) x
...
countdown x = "Ready! " ++ valiosa x ++ " Liftoff!"

valiosa :: Integer -> String
valiosa x = if x == 0 then "" else show x ++ "... " ++ valiosa (x-1)
...
smallestDivisor x = if (x `mod` 2 ) > 0 then helper x 2 else 2

helper :: Integer -> Integer -> Integer
helper x y = if (x `mod` y) == 0 then y else helper x (y+1)
