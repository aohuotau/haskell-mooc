...
add a (Set xs) = Set (sort(nub(a:xs)))
...
data State = Start | AddedEggs | AddedFlour | AddedSugar | AddedFlourSugar | Mixed | Error | Finished
  deriving (Eq,Show)


step :: State -> Event -> State
step state e = case (state, e) of
    (Start, AddEggs) -> AddedEggs
    (AddedEggs, AddFlour) -> AddedFlour
    (AddedEggs, AddSugar) -> AddedSugar
    (AddedFlour, AddSugar) -> AddedFlourSugar
    (AddedSugar, AddFlour) -> AddedFlourSugar
    (AddedFlourSugar, Mix) -> Mixed
    (Mixed, Bake) -> Finished
    (Finished, _) -> Finished 
    _ -> Error
