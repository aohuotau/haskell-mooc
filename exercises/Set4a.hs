...
distinct [] = True
distinct(x:xs) = if x `elem` xs then False else distinct xs
...
longest :: Ord a => [[a]] -> [a]
longest xs = minimumBy compareLength xs
    where
...
freqs xs = foldr (\x -> Map.alter (\v -> Just (1 + (case v of Nothing -> 0; Just n -> n))) x) Map.empty xs
...
transfer from to amount bank = if Map.notMember from bank || Map.notMember to bank || amount < 0 || Map.findWithDefault 0 from bank < amount then bank else Map.adjust (\x -> x - amount) from (Map.adjust (+ amount) to bank)
...
maxIndex arr = fst $ maximumBy (\(_, a) (_, b) -> compare a b) $ assocs arr